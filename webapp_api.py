...
def _level_unlocked(level: int, completed_levels: set[int], perfect_counts: dict[int, int]) -> bool:
    """Единое правило открытия уровня — используется и в /api/levels_status
    (что показать на сайте), и в /api/start_test (что реально разрешить
    сохранить как TestResult), намеренно в ОДНОМ месте: раньше эти две
    проверки жили порознь и разошлись, из-за чего можно было обойти
    разблокировку через кнопку "Следующий уровень" в обход интерфейса.

    Правило одинаково для ЛЮБОЙ должности (повар, бармен, официант —
    архитектура общая, ничего не хардкожено под конкретную должность):
    - Уровни 1-3 открыты всегда.
    - Уровень 4 открывается, когда пройдены (хотя бы по одному
      завершённому тесту, результат неважен) уровни 1, 2 и 3.
    - Уровень 5 и дальше открывается, только когда НЕПОСРЕДСТВЕННО
      предыдущий уровень пройден ИДЕАЛЬНО (без единой ошибки, включая
      дополнительный вопрос-"хвост", если он был показан на том уровне)."""
    if level <= 3:
        return True
    if level == 4:
        return {1, 2, 3}.issubset(completed_levels)
    return perfect_counts.get(level - 1, 0) > 0
...
async def levels_status(request: web.Request) -> web.Response:
    ...
    Правило открытия (см. _level_unlocked — единая функция, используется
    и здесь, и в /api/start_test, чтобы эти две проверки не могли
    разойтись):
    - Уровни 1-3 открыты всегда.
    - Уровень 4 открывается, когда ПОЛНОСТЬЮ пройдены (хотя бы по одному
      завершённому тесту, результат неважен) уровни 1, 2 и 3.
    - Уровень 5 и выше открывается, только когда НЕПОСРЕДСТВЕННО
      предыдущий уровень пройден ИДЕАЛЬНО — без единой ошибки (включая
      дополнительный вопрос-"хвост", если он был показан).

    Условие монотонно (основано на фактах прохождения, а не на текущем
    состоянии), поэтому уже открытый уровень никогда не блокируется
    обратно.
    ...
    него ещё не добавили вопросы (например, у бармена уровни 8+ пока
    пустые).

    perfect_count (0..3) — сколько раз уровень пройден ПОЛНОСТЬЮ без
    единой ошибки (капается на 3). Используется и для зелёной подсветки
    "N/3" на кнопке уровня, и как условие открытия следующего уровня
    (см. выше). Не путать с completed (там неважен результат, тут важна
    безошибочность)."""
    init_data = request.query.get("initData", "")
    tg_user = await _get_telegram_user(init_data)
    if tg_user is None:
    ...
    for lvl in range(1, MAX_QUESTION_LEVEL + 1):
        unlocked = _level_unlocked(lvl, completed_levels, perfect_counts)
        levels.append(
            {
                "level": lvl,
                "unlocked": unlocked,
...
async def start_test(request: web.Request) -> web.Response:
    ...
    async with async_session() as session:
        ...
        # (или воспользоваться багом на клиенте) с любым level, минуя
        # реальное прохождение. Правило — см. _level_unlocked (единая
        # функция с /api/levels_status, чтобы эти проверки не могли
        # снова разойтись).
        if level > 3:
            completed_levels = await crud.get_completed_levels_for_category(session, user.id, category_id)
            perfect_counts = await crud.get_perfect_pass_counts_for_category(session, user.id, category_id)
            if not _level_unlocked(level, completed_levels, perfect_counts):
                return _json(
